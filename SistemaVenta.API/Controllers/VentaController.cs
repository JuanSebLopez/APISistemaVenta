...
namespace SistemaVenta.API.Controllers
{
    ...
    public class VentaController : ControllerBase
    {
        ...
        public async Task<IActionResult> Registrar([FromBody] VentaDTO producto)
        {
            ...
            {
                rsp.status = true;
                rsp.value = await _ventaService.Register(producto);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Historial(string buscarPor, string? numeroVenta, string? fechaInicio, string? fechaFin)
        {
            ...
            {
                rsp.status = true;
                rsp.value = await _ventaService.Record(buscarPor, numeroVenta, fechaInicio, fechaFin);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Reporte(string? fechaInicio, string? fechaFin)
        {
            ...
            {
                rsp.status = true;
                rsp.value = await _ventaService.Report(fechaInicio, fechaFin);
            }
            catch (Exception ex)
            {
                rsp.status = false;
